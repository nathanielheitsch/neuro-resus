...
// this wham_lock prevents threads from printing on top of each other

omp_lock_t wham_lock;
...
void grabInsertLengths(string & targetfile){
  ...
  omp_set_lock(&wham_lock);
  int quals [126];
  memcpy(quals, SangerLookup, 126*sizeof(int));
  omp_unset_lock(&wham_lock);
  ...
  while(i < 5 || n < 100000){
    ...
    if(max < 2){
      max = 2; // single-seqid bam: rand() % (max-1) would be a division by zero
    }
