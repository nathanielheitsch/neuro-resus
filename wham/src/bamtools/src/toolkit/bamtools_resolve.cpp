                                                      ...
                                                      map<string, ReadGroupResolver>& readGroups)
{
    ...
    readGroups.insert( std::pair<std::string, ReadGroupResolver>(name, resolver) );
    return true;
}
...
bool ResolveTool::ResolveToolPrivate::MakeStats(void) {
    ...
    while ( bamReader.GetNextAlignmentCore(al) ) {
        ...
        else resolver.ReadNames.insert( std::pair<std::string, bool>(al.Name, isCurrentMateUnique) );
    }
...
void ResolveTool::ResolveToolPrivate::ParseHeader(const SamHeader& header) {
    ...
    for ( ; rgIter != rgEnd; ++rgIter ) {
        ...
        m_readGroups.insert( std::pair<std::string, ReadGroupResolver>(rg.ID, ReadGroupResolver()) );
    }
}
...
bool ResolveTool::ResolveToolPrivate::Run(void) {
    ...
    m_readGroups.insert( std::pair<std::string, ReadGroupResolver>("", ReadGroupResolver()) );
